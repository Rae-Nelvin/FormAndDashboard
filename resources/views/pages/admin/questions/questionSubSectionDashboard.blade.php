        ...
        <div class="flex flex-col w-full space-y-10">
            @foreach ($data as $datas)
                <div class="flex flex-row justify-between items-center bg-white w-full py-5 px-12 rounded-lg cursor-pointer hover:bg-teal-600 hover:text-white transition-all ease-in-out duration-300">
                    <a href="/package/section/subsection/question/renderQuestion/{{ $datas->id }}">
                ...
                <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="py-6 px-6 lg:px-8">
                            <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Edit Sub Section Soal</h3>
                            <form class="space-y-6" action="{{ route('editSubSection') }}" method="POST" >
                                @csrf
                                <input type="hidden" name="sectionQuestionID" value="{{ $sectionQuestionID }}">
                                <input type="hidden" name="subSectionID" value="{{ $datas->id }}">
                                <div>
                                    <label for="text" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Nama Sub Section Soal</label>
                ...
                <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="p-6 text-center">
                            ...
                            <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Apakah anda yakin ingin menghapus sub section soal ini?</h3>
                            <a data-modal-toggle="popup-modal-{{ $datas->id }}" href="/package/section/subsection/deleteSubSection/{{ $datas->id }}" type="button" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2 cursor-pointer">
                                Ya, saya yakin
                            </a>
    ...
    <div id="new-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full">
        <div class="relative p-4 w-full max-w-md h-full md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="py-6 px-6 lg:px-8">
                    <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">New Sub Section Soal</h3>
                    <form class="space-y-6" action="{{ route('newSubSection') }}" method="POST">
                        @csrf
                        <input type="hidden" name="sectionQuestionID" value="{{ $sectionQuestionID }}">
                        <div>
                            <label for="text" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Nama Sub Section Soal</label>
                            <input type="text" name="subSectionName" id="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Name Sub Section Soal" required>
                        </div>
                        <button type="submit" class="w-full text-white bg-green-500 hover:bg-green-600 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">New Sub Section Soal</button>
                    </form>
                </div>
            </div>
        </div>
